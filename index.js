...
class OrderBook {
  ...
  updateFirstAskAndBid() {
    ...
    if (newFirstAsk !== this.firstAsk || newFirstBid !== this.firstBid) {
      ...
      process.stdout.write(diffColor(newFirstAsk.price - this.firstAsk.price, true)(`${this.market.quote} ${parseFloat(newFirstAsk.price).toFixed(2)}`));
      ...
      process.stdout.write(diffColor(newFirstBid.price - this.firstBid.price)(`${this.market.quote} ${parseFloat(newFirstBid.price).toFixed(2)}`));
      process.stdout.write(' ');
      ...
      process.stdout.write(diffColor(newDifference - this.difference)(`${this.market.quote} ${newDifference.toFixed(2)}`));
      process.stdout.write('\n');
...
const orderBook = new OrderBook({ id: 'XBTUSD', base: 'XBT', quote: 'USD' });
